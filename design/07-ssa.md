...
SSA 是分析流水线的**中间步骤**，是结构性构建而非漏洞检测，不产出漏洞报告，因此不适用端到端误报率阈值（见 AGENTS.md 第 8 条），以**正确性**口径验收：CFG、φ 与值来源必须与 golden fixture 精确一致，系统失败必须为零，不得产生错误结构。正报为正确构建的函数体，负例为不产生错误 CFG、错误 φ 或错误值来源的结构。最低标准为存在正报，任何结构错误都是必须修复的缺陷，目标是零错误。
...
- 存在有效正报，CFG、φ 与值来源与 golden fixture 精确一致，系统失败为零，无错误结构（零错误）。
