...
Common AST 是分析流水线的**中间步骤**，不产出漏洞报告，因此不适用端到端误报率阈值（见 AGENTS.md 第 8 条），以**正确性**口径验收：核心节点映射必须与 golden fixture 精确一致，系统失败必须为零，不得产生错误结构。正报为正确映射的核心节点结构，负例为不产生无依据的错误结构。最低标准为五种语言均有正报，任何结构错误都是必须修复的缺陷，目标是零错误，且不得通过删除困难 fixture 降低标准。
...
- 五种语言均有有效正报，核心节点映射与 golden fixture 精确一致，系统失败为零，无错误结构（零错误）。
